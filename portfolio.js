(function () {
  ...
  const i18n = {
    pt: {
      ...
      project_shield_desc :"ShieldCall é um aplicativo Android desenvolvido para bloquear chamadas de números desconhecidos de forma automática, com foco em reduzir spam telefônico e melhorar a privacidade do usuário. O app atua localmente no dispositivo, sem depender de internet para sua função principal, e permite aplicar regras de bloqueio, manter listas de permissão e bloqueio, registrar eventos internamente e enviar SMS automático como resposta em cenários configurados. O projeto foi construído em Kotlin, com Jetpack Compose, arquitetura MVVM, Room e DataStore, utilizando as APIs oficiais do Android para triagem de chamadas. O diferencial do ShieldCall está em oferecer controle local, interface moderna e uma proposta prática de utilidade real, sem depender de anúncios ou serviços externos.",
    ...
    en: {
      ...
      project_shield_desc: "ShieldCall is an Android application developed to automatically block calls from unknown numbers, with a focus on reducing phone spam and improving user privacy. The app runs locally on the device, without relying on internet access for its core functionality, and allows users to apply blocking rules, manage whitelist and blacklist entries, keep an internal event log, and send automatic SMS replies in configured scenarios. The project was built with Kotlin, Jetpack Compose, MVVM architecture, Room, and DataStore, using official Android call screening APIs. Its main differentiator is offering local control, a modern interface, and a practical real-world solution without depending on ads or external services.",
